...
keep_negatives:
	...
	# move the number of element in the array to EDX
	movl 	num, 		%edx

	# move the current index to ECX
	movl 	$0,			%ecx

	# move the pointer of the array to ESI
	movl	ptrvec, 	%esi
...
start_loop:
	...
	# move the first number of the array to EDI
	movl	(%esi), 	%edi

	# check is number is negative
	cmpl 	$0, 	%edi

	# jump to the start if the number is negative
	jle next_number

	# change the number to his index if it was positive
	movl	%ecx, 	(%esi)
